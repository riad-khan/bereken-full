        ...
        <div class="w-full z-40 top-0 absolute md:relative text-gray-700 bg-white">
            <div x-data="{ open: false }" class="flex flex-col max-w-screen-xl px-2 mx-auto md:items-center md:justify-between md:flex-row md:px-0 ">
              ...
              <nav :class="{'flex': open, 'hidden': !open}" class="flex-col flex-grow pb-4 md:pb-0 hidden md:flex md:justify-end md:flex-row">
                ...
                <a class="py-2 my-2 mx-2 text-sm font-semibold text-[#2B313B] bg-white rounded-lg md:mt-0 hover:text-[#0052fe] " href="/knowledge">Calculations</a> 
                ...
                <div x-on:mouseover="open = true" x-on:mouseleave="open = false" class="" x-data="{ open: false }">
                  <button x-on:mouseover="open = true" x-on:mouseleave="open = false"  class="flex flex-row items-center justify-between w-full py-2 my-2 mx-2 text-sm font-semibold text-left text-[#2B313B] bg-transparent md:w-auto md:inline md:mt-0 md:ml-4 hover:text-[#0052fe] ">
                    <span><a href="www.google.com">Knowledge</a></span>
                    <svg fill="currentColor" viewBox="0 0 20 20" :class="{'rotate-180': open, 'rotate-0': !open}" class="inline w-4 h-4 mt-1 mr-2 transition-transform duration-200 transform md:-mt-1"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                  </button>
                  <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 w-full origin-top-right rounded-md shadow-lg border-t border-[#F5F8FF]">
                    ...
                    <button class="absolute right-2 top-2 md:hidden" @click="open = false"><img src="{{asset('frontend/assets/img/close.svg')}}" alt="icon"></button>
                        <div class="grid py-5 px-4 md:mx-auto max-w-screen-xl gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-6 md:px-4">
